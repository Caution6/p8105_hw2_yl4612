...
These data have not been tidy yet. For example, route 1 to 11 occupy 11 columns. If we could arrange them, dataset will be more tidy.
...
* `r nrow(filter(NYC_df, vending == "NO", entry == "TRUE")) / nrow(filter(NYC_df, vending == "NO"))` of station entrances/exits without vending allow entrance.
...
`r filter(re_NYC, route_name == "A", ada == "TRUE") %>% distinct(station_name, line) %>% nrow()` Of the stations that serve the A train, stations are ADA compliant.
